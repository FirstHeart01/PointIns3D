...
from pointins3d.model.modules.transformer_encoder import TransformerEncoder
from pointins3d.ops import voxelization, voxelization_idx
...
class GFM(nn.Module):
    ...
    def forward(self, p, v):
        # p: (n, c)
        # v: (m, c)
        p_weight = self.point_branch(p)  #(n, 2)
        v_weight = self.voxel_branch(v) #(m, 2)
        
        all = p_weight + v_weight
        weight_map = F.softmax(all, dim=-1)
        p_weight = weight_map[:,0].unsqueeze(1) 
        v_weight = weight_map[:,1].unsqueeze(1)
        fuse = v * v_weight + p * p_weight # (m, c)
        p = fuse #(m,c)
        # 形状没有改变，因为只是一个Linear线形层
        v = v.replace_feature(fuse)
        return p, v
...
class Res16UNetBase(Model):
    ...
    def __init__(self, in_channels, out_channels, train_cfg,
        ...
        d_model = 128
        self.input_trans_layer = nn.Linear(self.cs[1], d_model)
        self.point_branch = TransformerEncoder(d_model=d_model, N=2, heads=4, d_ff=512)
        self.output_trans_layer = nn.Linear(d_model, self.cs[4])
        self.point_stem = nn.ModuleList([
            nn.Sequential(nn.Linear(3, self.cs[1]), nn.BatchNorm1d(self.cs[1]),nn.GELU()),
            nn.Sequential(nn.Linear(self.cs[1], self.cs[2]), nn.BatchNorm1d(self.cs[2]),nn.GELU()),
            # SharedTransformer(3, self.cs[1]), # 3 => 32
            # SharedTransformer(self.cs[1], self.cs[2]), # 32  =>  256
            SharedTransformer(self.cs[2], self.cs[3]), # 256  =>  128
            SharedTransformer(self.cs[3], self.cs[4]), # 128  => 96
        ])
        self.gfm = GFM(self.cs[4])
        self.gfm_stem = GFM(self.cs[1])
        self.gfm_stage4 = GFM(self.cs[2])
        self.gfm_stage6 = GFM(self.cs[3])
        self.gfm_stage8 = GFM(self.cs[4])
    ...
    def forward(self, x, feature_maps):
        coords, coords_float, feats, offset, voxel_coords, spatial_shape, batch_size, v2p_map, p2v_map = x
        
        voxel_feats = voxelization(feats, p2v_map)
        voxel_input = spconv.SparseConvTensor(voxel_feats, voxel_coords.int(), spatial_shape, batch_size)
        
        out_p1 = self.input_layer(voxel_input) #(m, c)

        # 点特征直接送入到point_stage中，in_channel为32，out_channel为96，再融合
        # self.gfm(final_voxel_feats, self.point_branch(coords_float, points_feats, batch_ids))
        
        out_b1p2 = self.encoder1(out_p1) 
        out_b2p4 = self.encoder2(out_b1p2)
        out_b3p8 = self.encoder3(out_b2p4)
        out = self.encoder4(out_b3p8) #(m', 16c)
        feature_maps.append(out)
        # pixel_dist=8
        # TODO
        out = self.decoder4(out)
        out_feats = torch.cat((out.features, out_b3p8.features), dim=1)
        out = out.replace_feature(out_feats)
        out = self.block5(out)
        feature_maps.append(out)
        # pixel_dist=4
        ...
        out = self.block8(out)
        feature_maps.append(out)
        # 融合特征？将来自点分支的全局特征送入到最后
        batch_ids = coords[:, 0]
        points = feats
        before_params_feats = self.input_trans_layer(points)
        point_feats = self.point_branch(xyz=coords_float, feats=before_params_feats, batch_ids=batch_ids) 
        point_feats = self.output_trans_layer(point_feats)    
        fuse = self.gfm(out, point_feats)
...
class UBlock(nn.Module):
    ...
    def __init__(self, nPlanes, norm_fn, block_reps, block,with_transformer=False, indice_key_id=1):
        ...
        if len(nPlanes) <= 2 and with_transformer:
            d_model = 128
