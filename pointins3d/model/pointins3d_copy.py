...
from .blocks_copy import MLP, BasicBlock, Res16UNet34C, UBlock, ResidualBlock
...
class PointIns3D(nn.Module):
    ...
    def __init__(self,
        ...
        # block_channels = [channels * (i + 1) for i in range(7)]
        ...
        self.unet = Res16UNet34C(
            in_channels=in_channels, out_channels=channels, train_cfg=train_cfg, norm_fn=norm_fn
        )
    ...
    @cuda_cast
    def forward_train(self, coords, offset, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
                      semantic_labels, instance_labels, instance_pointnum, instance_cls,
                      pt_offset_labels, spatial_shape, batch_size, **kwargs):
        losses = {}
        if self.with_coords:
            feats = torch.cat((feats, coords_float), 1) #(n, 3 or 6)
        # origin_feats = feats
        ...
        semantic_scores, pt_offsets, output_feats = self.forward_backbone(coords, coords_float, feats, offset, 
                                                                          voxel_coords, spatial_shape, batch_size,
                                                                          v2p_map, p2v_map)
        # point wise losses
    ...
    def forward_backbone(self,coords, coords_float, feats, offset, voxel_coords,
                         spatial_shape, batch_size, v2p_map, p2v_map, x4_split=False):
        if x4_split:
            output_feats = self.forward_4_parts(input, v2p_map)
            output_feats = self.merge_4_parts(output_feats)
        else:
            aux = []
            input = (coords, coords_float, feats, offset, voxel_coords, spatial_shape, batch_size, v2p_map, p2v_map)
