...
class WindowAttention3D(spconv.SparseModule):
    ...
    def forward(self, x, mask=None, prev_v=None, prev_k=None, prev_q=None, is_decoder=False):
        ...
        relative_position_bias = self.relative_position_bias_table[
            ...
            N, N, -1)  # Wd*Wh*Ww,Wd*Wh*Ww,nH # 这里跟PVT有所不同
...
class SwinTransformerBlock3D(spconv.SparseModule):
    ...
    def forward_part1(self, x, mask_matrix, prev_v, prev_k, prev_q, is_decoder):
        tensor_x = x.dense()
        B, D, H, W, C = tensor_x.shape
        window_size, shift_size = get_window_size((D, H, W), self.window_size, self.shift_size)
        
        tensor_x = self.norm1(tensor_x)
        # pad feature maps to multiples of window size
        pad_l = pad_t = pad_d0 = 0
        pad_d1 = (window_size[0] - D % window_size[0]) % window_size[0]
        pad_b = (window_size[1] - H % window_size[1]) % window_size[1]
        pad_r = (window_size[2] - W % window_size[2]) % window_size[2]
        tensor_x = F.pad(tensor_x, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_d0, pad_d1))
    ...
    def forward(self, x, mask_matrix, prev_v, prev_k, prev_q, is_decoder=False):
        ...
        x = shortcut + self.drop_path(x) # PVT 这里*0.5

        if self.use_checkpoint:
            x = x + checkpoint.checkpoint(self.forward_part2, x)
        else:
            x = x + self.forward_part2(x) # 同样这里*0.5了
...
class BasicLayer(spconv.SparseModule):
    """ A basic Swin Transformer layer for one stage.
    Args:
    ...
    def forward(self, x, block_num):
        ...
        # calculate attention mask for SW-MSA
        tensor_x = x.dense()
        B, C, D, H, W = tensor_x.shape
        window_size, shift_size = get_window_size((D, H, W), self.window_size, self.shift_size)
        tensor_x = rearrange(tensor_x, 'b c d h w -> b d h w c')

        Dp = int(np.ceil(D / window_size[0])) * window_size[0]
        Hp = int(np.ceil(H / window_size[1])) * window_size[1]
        Wp = int(np.ceil(W / window_size[2])) * window_size[2]
        attn_mask = compute_mask(Dp, Hp, Wp, window_size, shift_size, tensor_x.device)
        ...
        tensor_x = x.dense()
        tensor_x = tensor_x.reshape(B, D, H, W, -1)
        # x = x.reshape(B, D, H, W, -1)
        x = spconv.SparseConvTensor.from_dense(tensor_x) 
        if self.downsample is not None:
            x = self.downsample(x)
...
class PatchEmbed3D(spconv.SparseModule):
    ...
    def __init__(self, img_size=(128, 128, 128), patch_size=(4, 4, 4), in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.proj2 = nn.Conv3d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
        # 可以加一个todense()层
        self.proj = spconv.SubMConv3d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
    ...
    def forward(self, x):
        ...
        # padding
        tensor_x = x.dense()
        _, _, D, H, W = tensor_x.size()
        if W % self.patch_size[2] != 0:
            tensor_x = F.pad(tensor_x, (0, self.patch_size[2] - W % self.patch_size[2]))
        if H % self.patch_size[1] != 0:
            tensor_x = F.pad(tensor_x, (0, 0, 0, self.patch_size[1] - H % self.patch_size[1]))
        if D % self.patch_size[0] != 0:
            tensor_x = F.pad(tensor_x, (0, 0, 0, 0, 0, self.patch_size[0] - D % self.patch_size[0]))
        start, end = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
        start.record()
        x2 = self.proj2(tensor_x)
        torch.cuda.synchronize()
        end.record()
        print(start.elapsed_time(end))
        x = spconv.SparseConvTensor.from_dense(tensor_x.permute(0,2,3,4,1))
        start.record()
        ...
        if self.norm is not None:
            D, Wh, Ww = tensor_x.size(2), tensor_x.size(3), tensor_x.size(4)
            tensor_x = tensor_x.flatten(2).transpose(1, 2) # [B, N ,C]
            tensor_x = self.norm(tensor_x)
            tensor_x = tensor_x.transpose(1, 2).view(-1, self.embed_dim, D, Wh, Ww) # [B, C, D, Wh, Ww]
            x = spconv.SparseConvTensor.from_dense(tensor_x.permute(0,2,3,4,1))
        return x


class SwinTransformerSys3D(spconv.SparseModule):
    r""" Swin Transformer
    ...
    def forward_features(self, x):
        x = self.patch_embed(x) # tensor数据为 B' embed D Wh Ww
        # x = self.pos_drop(x)
        x_feats = self.pos_drop(x.dense()) 
        x = spconv.SparseConvTensor.from_dense(x_feats.permute(0,2,3,4,1))
        x_downsample = []
        v_values_1 = []
        k_values_1 = []
        q_values_1 = []
...
class PTUNet(spconv.SparseModule):
    def __init__(self, in_chans=3, num_heads=[3,6,12,24], zero_head=False, embed_dim=48, win_size=7):
        super(PTUNet, self).__init__()
