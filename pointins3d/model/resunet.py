...
from pointins3d.model.resnet import ResNetBase, get_norm
from pointins3d.model.modules.common import ConvType, NormType, conv, conv_tr
from pointins3d.model.modules.resnet_block import BasicBlock, Bottleneck, BasicBlockINBN
...
class MinkUNetBase(ResNetBase):
    ...
    def __init__(self, in_channels, out_channels, train_config, D=3, **kwargs):
        super().__init__(in_channels, out_channels, train_config, D)

    def network_initialization(self, in_channels, out_channels, train_config, D):
        # Setup net_metadata
        ...
        self.conv1p1s1 = conv(
            in_channels,
            self.inplanes,
            kernel_size=space_n_time_m(train_config.conv1_kernel_size, 1),
            stride=1,
            dilation=1,
...
class MinkUNetHyper(MinkUNetBase):
    ...
    # Once data is processed, call clear to reset the model before calling initialize_coords
    def __init__(self, in_channels, out_channels, train_config, D=3, **kwargs):
        super(MinkUNetBase, self).__init__(in_channels, out_channels, train_config, D)
