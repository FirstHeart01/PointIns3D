...
ConnectedComponent find_cc2(Int idx, int *semantic_label, Int *ball_query_idxs, int *start_len,
                           int *visited) {
  ConnectedComponent cc;
  cc.addPoint(idx);
  visited[idx] = 1;
  ...
  while (!Q.empty()) {
    ...
    int start = start_len[cur * 2];
    int len = start_len[cur * 2 + 1];
    int label_cur = semantic_label[cur * 2 + 1];
    for (Int i = start; i < start + len; i++) {
      Int idx_i = ball_query_idxs[i];
...
int get_clusters2(int *semantic_label, int *ball_query_idxs,
                 int *start_len, const int nPoint, float threshold,
  ...
  for (int i = 0; i < nPoint; i++) {
    if (visited[i] == 0) {
      ConnectedComponent CC = find_cc2(i, semantic_label, ball_query_idxs, start_len, visited);
      // _class_numpoint_mean = class_numpoint_mean[class_id];
      ...
      //   thr = threshold * _class_numpoint_mean;
      // }
      if ((int)CC.pt_idxs.size() >= threshold*10000) {
        clusters.push_back(CC);
        sumNPoint += (int)CC.pt_idxs.size();
      }
    }
...
void bfs_cluster2(at::Tensor semantic_label_tensor,
                 at::Tensor ball_query_idxs_tensor, at::Tensor start_len_tensor,
                 at::Tensor cluster_idxs_tensor,
                 at::Tensor cluster_offsets_tensor, const int N,
                 float threshold) {
  ...
  ConnectedComponents CCs;
  int sumNPoint = get_clusters2(semantic_label, ball_query_idxs, start_len, N, threshold, CCs);
