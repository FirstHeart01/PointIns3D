...
__global__ void get_mask_label_cuda_(int nInstance, int nProposal,
  ...
  for (int proposal_id = blockIdx.x; proposal_id < nProposal;
    ...
    for (int instance_id = 0; instance_id < nInstance; instance_id++) {
      // proposals_iou是一个指针
      // 这表示我选择了我预测的proposal_id的实例与instance_id的iou的值是多少
      // 这部分代码相当于在筛选我预测的proposal与哪个instance_id的iou最大，然后就认为proposal为这个实例id
    ...
    if (max_iou >= iou_thr) {
      for (int i = start; i < end; i++) {
        // 这部分就开始处理预测的proposal中的点的mask，如果点的实例id不等于真实实例id，那么就mask为0
        int idx = proposals_idx[i];
