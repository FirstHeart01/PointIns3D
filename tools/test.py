...
from pointins3d.data import build_dataloader, build_dataset
from pointins3d.evaluation import (PanopticEval, ScanNetEval, evaluate_offset_mae,
                                  evaluate_semantic_acc, evaluate_semantic_miou)
from pointins3d.model import PointIns3D
from pointins3d.util import (collect_results_cpu, get_dist_info, get_root_logger, init_dist,
...
def get_args():
    parser = argparse.ArgumentParser('PointIns3D')
...
def main():
    ...
    with torch.no_grad():
        ...
        for i, time in enumerate(times):
            print('iterate{}: inference time:{:4f}'.format(i, time))
        print('avg time:{:4f}'.format(np.array(times).sum()/len(times)))
        print('max time:{:4f}'.format(np.array(times[1:]).max()))
    if is_main_process():
        for res in results:
